...
with DAG(
    ...
    params={
        "year": Param(
            2024, 
            type="integer", 
            title="Processing Year",
            description="Reference year for the execution (not strictly used by dbt as models are views)"
        ),
    },
) as dag:

    # 1. dbt debug: Verify connection (uses --target to point at correct DW)
